            ...
            <div onClick={(e) => e.stopPropagation()} className="modal w-full max-w-5xl">
                ...
                <div className="grid gap-6 md:grid-cols-[1fr_360px]">
                        ...
                        {cart.map((i) => (
                            <div key={i.id} className="flex items-center justify-between border-b pb-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-1 flex items-center gap-2 text-sm">
                                            <button
                                                ...
                                                onClick={() =>
                                                    setCart((c) =>
                                                        c.map((x) => (x.id === i.id ? { ...x, qty: Math.max(1, x.qty - 1) } : x))
                                                    )
                                                }
                                            >
                                                –
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <button className="text-xl" onClick={() => setCart((c) => c.filter((x) => x.id !== i.id))}>
                                        ×
                                    </button>
                                </div>
                            </div>
                    ...
                    <div className="rounded-2xl bg-[var(--brand-card)] p-5">
                        ...
                        <button
                            className="w-full rounded-xl bg-[var(--brand-rose)] px-6 py-3 font-semibold text-white shadow"
                            onClick={onCheckout}
                        >
                            Оформить заказ
                        </button>
